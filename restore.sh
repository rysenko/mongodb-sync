...
[[ -z "${1}" ]] && FILE_TO_RESTORE=$(ls /backup -w1 | grep -iv ".tgz" | sort -r | head -n 1)

echo "=> Restore database from ${FILE_TO_RESTORE}"
if mongorestore --drop --host ${MONGODB_RESTORE_HOST} --port ${MONGODB_RESTORE_PORT} ${USER_RESTORE_STR}${PASS_RESTORE_STR}${DB_RESTORE_STR} ${EXTRA_RESTORE_OPTS} /backup/$FILE_TO_RESTORE/${MONGODB_BACKUP_DB}; then
